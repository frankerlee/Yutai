...
using Yutai.Services.Serialization.Legacy;
using Yutai.Shared;
...
namespace Yutai.Services.Concrete
{
    public class ProjectLoaderLegacy : ProjectLoaderBase
    {
      /*  private readonly ImageSerializationService _imageSerializationService;
        ...
        /// </summary>
        public bool Restore(YutaiProject project, string filename)
        {
            if (project == null)
            {
                return false;
       ...
       /* private void RestoreExtents(YutaiProject project)
        {
            ...
            try
            {
              /*  var env = new Envelope(double.Parse(state.ExtentsLeft),
                ...
                _context.Map.ZoomToExtents(env);#1#
            }
            catch (Exception ex)
            {
       ...
       /* private void RestoreMapProjection(YutaiProject project, string filename)
        {
