...
namespace Yutai.ArcGIS.Common.BaseClasses
{
	public class WorkspaceOperator
	{
		...
		public static IFeatureClass CreateAnnoFeatureClass(string string_0, IFeatureDataset ifeatureDataset_0, double double_0)
		{
			...
			IFeatureClassDescription featureClassDescription = (IFeatureClassDescription)annotationFeatureClassDescriptionClass;
			...
			IFeatureClass featureClass = workspace.CreateAnnotationClass(string_0, requiredFields, instanceCLSID, classExtensionCLSID, featureClassDescription.ShapeFieldName, "", ifeatureDataset_0, null, null, graphicsLayerScaleClass, symbolCollectionClass, true);
			return featureClass;
		}
		...
		public static IWorkspace CreateFileGDB(string string_0)
		{
			...
			try
			{
				IWorkspaceName workspaceName = fileGDBWorkspaceFactoryClass.Create(System.IO.Path.GetDirectoryName(string_0), System.IO.Path.GetFileNameWithoutExtension(string_0), null, 0);
		...
		public static IWorkspace CreateLocalGDB(string string_0)
		{
			...
			workspace = (System.IO.Path.GetExtension(string_0).ToLower() != ".mdb" ? WorkspaceOperator.CreateFileGDB(string_0) : WorkspaceOperator.CreatePersonGDB(string_0));
			return workspace;
		}
		...
		public static IWorkspace CreatePersonGDB(string string_0)
		{
			...
			try
			{
				IWorkspaceName workspaceName = accessWorkspaceFactoryClass.Create(System.IO.Path.GetDirectoryName(string_0), System.IO.Path.GetFileNameWithoutExtension(string_0), null, 0);
				workspace = (workspaceName as IName).Open() as IWorkspace;
			}
			catch (COMException cOMException1)
			{
        ...
        public static string GetWorkspaceConnectInfo(IWorkspace iworkspace_0)
		{
			...
			if (iworkspace_0 != null)
			{
				...
				if ((iworkspace_0.Type == esriWorkspaceType.esriLocalDatabaseWorkspace ? false : iworkspace_0.Type != esriWorkspaceType.esriFileSystemWorkspace))
				{
					...
					lower = Convert.ToString(iworkspace_0.ConnectionProperties.GetProperty("DB_CONNECTION_PROPERTIES")).ToLower();
					stringBuilder.Append(lower);
					stringBuilder.Append(",");
		...
		{
			IWorkspaceProperty property = ((IWorkspaceProperties)iworkspace_0).Property[esriWorkspacePropertyGroupType.esriWorkspacePropertyGroup, 7];
		...
		public static bool IsConnectedToGeodatabase(IWorkspaceName iworkspaceName_0)
		{
			...
			try
			{
				...
				{
					geodatabase = WorkspaceOperator.IsConnectedToGeodatabase((iworkspaceName_0 as IName).Open() as IWorkspace);
					return geodatabase;
				}
			}
			catch (Exception exception)
			...
			else if (iworkspace_0.Type == iworkspace_1.Type)
			{
				...
				if ((iworkspace_0.Type == esriWorkspaceType.esriLocalDatabaseWorkspace ? false : iworkspace_0.Type != esriWorkspaceType.esriFileSystemWorkspace))
				{
					...
					if (lower == Convert.ToString(iworkspace_1.ConnectionProperties.GetProperty("DBCLIENT")).ToLower())
					{
						string str = lower;
						lower = Convert.ToString(iworkspace_0.ConnectionProperties.GetProperty("DB_CONNECTION_PROPERTIES")).ToLower();
						if (lower == Convert.ToString(iworkspace_1.ConnectionProperties.GetProperty("DB_CONNECTION_PROPERTIES")).ToLower())
						{
							if (str != "sqlserver")
							{
								flag1 = true;
							...
							{
								lower = Convert.ToString(iworkspace_0.ConnectionProperties.GetProperty("Database")).ToLower();
								string lower1 = Convert.ToString(iworkspace_1.ConnectionProperties.GetProperty("Database")).ToLower();
								flag1 = lower == lower1;
							}
						}
					}
