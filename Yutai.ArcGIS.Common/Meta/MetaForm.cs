...
namespace Yutai.ArcGIS.Common.Meta
{
	public class MetaForm
	{
		...
		public MetaForm(Form aForm, EmfType mfType, bool draw)
		{
			...
			this.mf = new Metafile(this.mfStream, hdc, new Rectangle(0, 0, aForm.Width, aForm.Height), MetafileFrameUnit.Pixel, mfType);
				...
				switch ((arr[i] as MetaForm.ObjectEnt).type)
				{
					case MetaForm.ObjTypeRecord.objCloseLine:
					{
						...
						for (j = 0; j <= count; j++)
						{
							if (j < count)
							{
								MetaForm.DrawLine((PointF)(arr[i] as MetaForm.ObjectEnt).points[j], (PointF)(arr[i] as MetaForm.ObjectEnt).points[j + 1]);
							}
						}
						j--;
						if ((arr[i] as MetaForm.ObjectEnt).compobj != -1)
						{
							...
							MetaForm.DrawLine((PointF)(arr[i] as MetaForm.ObjectEnt).points[j], (PointF)(arr[item] as MetaForm.ObjectEnt).points[0]);
							break;
						}
						else
						{
							...
							MetaForm.DrawLine((PointF)(arr[i] as MetaForm.ObjectEnt).points[j], (PointF)(arr[i] as MetaForm.ObjectEnt).points[0]);
							break;
						}
					}
					case MetaForm.ObjTypeRecord.objCloseBezier:
					{
						...
						if ((arr[i] as MetaForm.ObjectEnt).compobj != -1)
						{
							...
							MetaForm.DrawLine((PointF)(arr[i] as MetaForm.ObjectEnt).points[j], (PointF)(arr[item1] as MetaForm.ObjectEnt).points[0]);
							break;
						}
						else
						{
		...
		private static bool DelFunc(EmfPlusRecordType recordType, int flags, int dataSize, IntPtr recordData, PlayRecordCallback callbackData)
		{
		...
		public bool SplitingToStream(IntPtr Handle, StringWriter ASt)
		{
			...
			using (graphic)
			{
				graphic1.EnumerateMetafile(this.mf, new Point(0, 0), new Rectangle(0, 0, this.mf.Width, this.mf.Height), GraphicsUnit.Pixel, new Graphics.EnumerateMetafileProc(MetaForm.DelFunc));
			}
			graphic1.Dispose();
		...
		private static void SplitRecObject(byte[] curArr)
		{
			...
			while (num2 < length)
			{
				...
				if ((arrayLists[num3] as MetaForm.ObjectEnt).type == MetaForm.ObjTypeRecord.objBezier && curArr[num2] == 1 || (arrayLists[num3] as MetaForm.ObjectEnt).type == MetaForm.ObjTypeRecord.objLine && curArr[num2] == 3)
				{
					arrayLists.Add(new MetaForm.ObjectEnt());
					num3++;
					if (curArr[num2] == 1)
					...
					(arrayLists[num3] as MetaForm.ObjectEnt).points.Add((arrayLists[num3 - 1] as MetaForm.ObjectEnt).points[count]);
					(arrayLists[num3] as MetaForm.ObjectEnt).compobj = num3 - 1;
				}
				if (num1 != 4)
				{
					(arrayLists[num3] as MetaForm.ObjectEnt).points.Add(new PointF(MetaForm.FormatSize((int)curArr[num + 2], (int)curArr[num + 3]), MetaForm.FormatSize((int)curArr[num + 6], (int)curArr[num + 7])));
				}
				else
				{
					(arrayLists[num3] as MetaForm.ObjectEnt).points.Add(new PointF(MetaForm.Format_Size2(curArr[num], curArr[num + 1]), MetaForm.Format_Size2(curArr[num + 2], curArr[num + 3])));
				}
		...
		Label0:
			...
			using (graphic)
			{
				graphic1.EnumerateMetafile(MetaForm.mfImage, new Point(0, 0), new Rectangle(0, 0, MetaForm.mfImage.Width, MetaForm.mfImage.Height), GraphicsUnit.Pixel, new Graphics.EnumerateMetafileProc(MetaForm.DelFunc));
			}
			graphic1.Dispose();
