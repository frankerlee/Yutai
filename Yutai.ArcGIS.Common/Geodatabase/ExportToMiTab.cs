...
{
	public class ExportToMiTab : IConvertEvent, ESRI.ArcGIS.Geodatabase.IFeatureProgress_Event, IDataConvert, ITABConvert
	{
			...
			for (IFeature j = featureCursor.NextFeature(); j != null; j = featureCursor.NextFeature())
			{
				...
				if (j.Shape != null)
				{
					...
					if (num == 4)
					{
						...
						if (annotation != null)
						{
							double xMin = ((annotation as IElement).Geometry.Envelope.XMin + (annotation as IElement).Geometry.Envelope.XMax) / 2;
							double yMin = ((annotation as IElement).Geometry.Envelope.YMin + (annotation as IElement).Geometry.Envelope.YMax) / 2;
							...
							TabWrite._mitab_c_set_text_display(intPtr1, annotation.Symbol.Angle, annotation.Symbol.Size, annotation.Symbol.Size, annotation.Symbol.Color.RGB, 16777215, -1, -1, -1);
						}
					}
					else if (ifeatureClass_0.ShapeType == esriGeometryType.esriGeometryPoint)
					{
						...
						for (i = 0; i < geometryCollection.GeometryCount; i++)
						{
							...
							for (int k = 0; k < geometry.SegmentCount; k++)
							{
								...
								else if (segment != null)
								{
									...
									try
									{
										(polylineClass as ISegmentCollection).AddSegment((segment as IClone).Clone() as ISegment, ref obj, ref obj);
										polylineClass.Densify(0, 0);
									}
									catch (Exception exception)
									{
									}
									for (int l = (k == 0 ? 0 : 1); l < (polylineClass as IPointCollection).PointCount; l++)
									{
