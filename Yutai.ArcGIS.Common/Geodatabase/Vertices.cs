	...
	public class Vertices : EnumImpl
	{
		...
		public virtual Vertex this[int int_1]
		{
			get
			{
				...
				if (int_1 < base.Count)
				{
					vertex = this.CreateVertex(MiApi.mitab_c_get_vertex_x(this.Part.Feature.Handle, this.Part.Index, int_1), MiApi.mitab_c_get_vertex_y(this.Part.Feature.Handle, this.Part.Index, int_1));
				}
				else
				{
					vertex = null;
		...
		protected internal Vertices(Part part_0) : base(MiApi.mitab_c_get_vertex_count(part_0.Feature.Handle, part_0.Index))
		{
			this.Part = part_0;
		}
