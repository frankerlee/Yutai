	...
	internal class VersioningUtil
	{
		...
		public void GetDifferences1(IWorkspace iworkspace_0, string string_0, string string_1, TreeNodeCollection treeNodeCollection_0, IList ilist_0)
		{
			try
			{
				...
				this.CheckDatasetForDifferences(treeNodeCollection_0, iworkspace0, featureWorkspace, featureWorkspace1, string_0, string_1, ilist_0);
				if (!this.bool_0)
				{
		...
		private void method_0(IFeatureWorkspace ifeatureWorkspace_0, IFeatureWorkspace ifeatureWorkspace_1, string string_0, string string_1, IDataset idataset_0, IList ilist_0)
		{
			...
			this.method_5(featureClass as IVersionedTable, featureClass1 as IVersionedTable, string_0, string_1, idataset_0.Name, ilist_0);
		}
		...
		private void method_2(int int_0, esriDifferenceType esriDifferenceType_0, string string_0, string string_1, string string_2, IList ilist_0)
		{
			...
			ilist_0.Add(str);
			if ((esriDifferenceType_0 == esriDifferenceType.esriDifferenceTypeUpdateUpdate || esriDifferenceType_0 == esriDifferenceType.esriDifferenceTypeUpdateDelete ? true : esriDifferenceType_0 == esriDifferenceType.esriDifferenceTypeDeleteUpdate))
			{
				ilist_0.Add("发现冲突");
			}
			if ((esriDifferenceType_0 == esriDifferenceType.esriDifferenceTypeUpdateNoChange ? true : esriDifferenceType_0 == esriDifferenceType.esriDifferenceTypeUpdateUpdate))
			{
		...
		private void method_3(int int_0, string string_0, string string_1, IList ilist_0)
		{
			try
			{
				...
				for (int i = 0; i < feature.Fields.FieldCount; i++)
				{
					...
					{
						if ((feature.Fields.Field[i].Name.ToUpper() == upper || feature.Fields.Field[i].Name.ToUpper() == "SHAPE.AREA" ? false : !(feature.Fields.Field[i].Name.ToUpper() == "SHAPE.LEN")))
						{
		...
		private bool method_7(IVersionedWorkspace iversionedWorkspace_0, IEnumVersionInfo ienumVersionInfo_0, string string_0, bool bool_2, bool bool_3)
		{
			...
			try
			{
				for (IVersionInfo i = ienumVersionInfo_0.Next(); i != null; i = ienumVersionInfo_0.Next())
				{
					if (!bool_2)
					{
						if (this.method_8(iversionedWorkspace_0.FindVersion(i.VersionName) as IVersionEdit, string_0, bool_2, bool_3))
						{
					...
					else if (!this.method_8(iversionedWorkspace_0.FindVersion(i.VersionName) as IVersionEdit, string_0, bool_2, bool_3))
					{
						flag = true;
					}
				}
