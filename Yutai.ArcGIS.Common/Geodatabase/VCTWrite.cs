	...
	public class VCTWrite : IConvertEvent, ESRI.ArcGIS.Geodatabase.IFeatureProgress_Event, IProgressMessage
	{
		...
		public IDataset Dataset
		{
			set
			{
				...
				else if (value is IFeatureDataset)
				{
					...
					subsets.Reset();
					for (IFeatureClass i = subsets.Next() as IFeatureClass; i != null; i = subsets.Next() as IFeatureClass)
					{
						if ((i.FeatureType == esriFeatureType.esriFTSimple ? true : i.FeatureType == esriFeatureType.esriFTAnnotation))
						{
		...
		public bool AddDataset(IDataset idataset_1)
		{
			...
			else if (idataset_1 is IFeatureDataset)
			{
				...
				for (IDataset i = subsets.Next(); i != null; i = subsets.Next())
				{
					...
					{
						if ((featureClass.FeatureType == esriFeatureType.esriFTSimple ? true : featureClass.FeatureType == esriFeatureType.esriFTAnnotation))
						{
		...
		private void method_10(StreamWriter streamWriter_0)
		{
			...
			for (int i = 0; i < this.ilist_0.Count; i++)
			{
				...
				if (num != 0 && item.FeatureType == esriFeatureType.esriFTSimple && item.ShapeType == esriGeometryType.esriGeometryPolygon)
				{
					...
					for (IFeature j = featureCursor.NextFeature(); j != null; j = featureCursor.NextFeature())
					{
						...
						if ((shape == null ? false : !shape.IsEmpty))
						{
							...
							{
								IPointCollection geometry = (shape as IGeometryCollection).Geometry[k] as IPointCollection;
		...
		private void method_12(StreamWriter streamWriter_0, IFeature ifeature_0)
		{
			...
			if (!(ifeature_0 is IAnnotationFeature))
			{
				...
				for (i = 0; i < fields.FieldCount; i++)
				{
					...
					if ((field.Type == esriFieldType.esriFieldTypeOID || field.Type == esriFieldType.esriFieldTypeBlob || field.Type == esriFieldType.esriFieldTypeGeometry || field.Type == esriFieldType.esriFieldTypeRaster ? false : field.Editable) && !(field.Name == this.string_0))
					{
			...
			else
			{
				...
				{
					field = fields.Field[i];
					if ((field.Type == esriFieldType.esriFieldTypeOID || field.Type == esriFieldType.esriFieldTypeBlob || field.Type == esriFieldType.esriFieldTypeGeometry || field.Type == esriFieldType.esriFieldTypeRaster ? false : field.Editable) && !(field.Name == this.string_0) && this.ifields_0.FindField(field.Name) == -1)
					{
						value = ifeature_0.Value[i];
						str = (!(value is DBNull) ? string.Concat(str, ",", value.ToString()) : string.Concat(str, ","));
					}
		...
		private void method_8(StreamWriter streamWriter_0)
		{
			...
			for (int i = 0; i < this.ilist_0.Count; i++)
			{
				...
				int num = item.FeatureCount(spatialFilterClass);
				if (num != 0 && item.FeatureType == esriFeatureType.esriFTSimple && item.ShapeType == esriGeometryType.esriGeometryPoint)
				{
					string str = this.ilist_1[i].ToString();
					string name = (item as IDataset).Name;
					if (this.setFeatureClassNameEnventHandler_0 != null)
		...
		private void method_9(StreamWriter streamWriter_0)
		{
			...
			for (int i = 0; i < this.ilist_0.Count; i++)
			{
				...
				int num = item.FeatureCount(spatialFilterClass);
				if (num != 0 && item.FeatureType == esriFeatureType.esriFTSimple && item.ShapeType == esriGeometryType.esriGeometryPolyline)
				{
					string str = this.ilist_1[i].ToString();
					string name = (item as IDataset).Name;
					string[] strArrays = name.Split(new char[] { '.' });
		...
		internal void WriteHead(StreamWriter streamWriter_0)
		{
			...
			{
				str = string.Concat("Projection: ", (this.ispatialReference_0 as IProjectedCoordinateSystem).Projection.Name);
				streamWriter_0.WriteLine(str);
				spheroid = (this.ispatialReference_0 as IProjectedCoordinateSystem).GeographicCoordinateSystem.Datum.Spheroid;
