...
namespace Yutai.ArcGIS.Common.Carto
{
	public class Clip
	{
		...
		public static void ClipLayer(IFeatureLayer ifeatureLayer_0, IFeatureClass ifeatureClass_0, IFeatureWorkspace ifeatureWorkspace_0, IMap imap_0)
		{
			...
			try
			{
				...
				name = str;
				IFeatureClass featureClass = ifeatureWorkspace_0.CreateFeatureClass(name, field, null, null, esriFeatureType.esriFTSimple, name1, "");
				...
				IFeatureClass featureClass1 = basicGeoprocessorClass.Clip(ifeatureLayer_0 as ITable, false, ifeatureClass_0 as ITable, false, 0, fullName);
				if (imap_0 != null)
				{
		...
		public static void ClipMapByRegion(IWorkspaceName iworkspaceName_0, IMap imap_0, IGeometry igeometry_0, IMap imap_1)
		{
		...
		private static void ExtractSelectedFeatures(IFeatureWorkspace ifeatureWorkspace_0, ICompositeLayer icompositeLayer_0)
		{
		...
		public static void ExtractSelectFeatureFormFeatureCursor(ILayer ilayer_0, IFeatureCursor ifeatureCursor_0, IFeatureWorkspace ifeatureWorkspace_0, IGeometry igeometry_0, IMap imap_0)
		{
			IEnumFieldError enumFieldError;
			IFields field;
			string str;
				...
				while (true)
				{
					if (num < field.FieldCount)
					{
						...
						if (field1.Type == esriFieldType.esriFieldTypeGeometry)
						{
							...
							SpatialReferenctOperator.ChangeCoordinateSystem(ifeatureWorkspace_0 as IGeodatabaseRelease, spatialReference, false);
		...
		public static void ExtractSelectFeatureFormFeatureCursor(string string_0, IFeatureCursor ifeatureCursor_0, IFeatureWorkspace ifeatureWorkspace_0, IGeometry igeometry_0)
		{
			IEnumFieldError enumFieldError;
			IFields field;
			string str;
			...
			if (ifeatureCursor_0 != null)
			{
				...
				string_0 = str;
				if (((ifeatureWorkspace_0 as IWorkspace).Type == esriWorkspaceType.esriLocalDatabaseWorkspace ? false : (ifeatureWorkspace_0 as IWorkspace).Type != esriWorkspaceType.esriRemoteDatabaseWorkspace))
				{
				...
				try
				{
					IFeatureClass featureClass = ifeatureWorkspace_0.CreateFeatureClass(string_0, field, null, null, esriFeatureType.esriFTSimple, name, "");
					IFeature feature = ifeatureCursor_0.NextFeature();
					IFeatureCursor featureCursor = featureClass.Insert(true);
					IFeatureBuffer featureBuffer = featureClass.CreateFeatureBuffer();
					while (feature != null)
				...
				if (ifeatureLayer0.SelectionSet.Count != 0)
				{
					...
					for (int i = field.FieldCount - 1; i >= 0; i--)
					{
						...
						if (field3.Type == esriFieldType.esriFieldTypeGeometry)
						{
							...
							ISpatialReference spatialReference = (geometryDef.SpatialReference as IClone).Clone() as ISpatialReference;
							SpatialReferenctOperator.ChangeCoordinateSystem(ifeatureWorkspace_0 as IGeodatabaseRelease, spatialReference, false);
							(geometryDef as IGeometryDefEdit).SpatialReference_2 = spatialReference;
					...
					try
					{
						featureClass1 = ifeatureWorkspace_0.CreateFeatureClass(name1, fieldsClass, null, null, esriFeatureType.esriFTSimple, name2, "");
					}
					catch (Exception exception)
					{
		...
		private static void ExtractSpecifyHRegFeatures(IFeatureWorkspace ifeatureWorkspace_0, ICompositeLayer icompositeLayer_0, IGeometry igeometry_0)
		{
			for (int i = 0; i < icompositeLayer_0.Count; i++)
			{
				ILayer layer = icompositeLayer_0.Layer[i];
				if (layer.Visible)
				{
					...
					else if (layer is IFeatureLayer)
					{
						...
						if (!igeometry_0.IsEmpty)
						{
							Clip.ExtractSelectFeatureFormFeatureCursor(layer.Name, Clip.searchFeatureFormLayer(layer, igeometry_0), ifeatureWorkspace_0, igeometry_0);
						}
					}
				}
			}
		...
		public static void ExtractSpecifyHRegFeatures(IWorkspaceName iworkspaceName_0, IMap imap_0, IGeometry igeometry_0)
		{
			(new SelectionEnvironment()).CombinationMethod = esriSelectionResultEnum.esriSelectionResultNew;
			IWorkspace workspace = (iworkspaceName_0 as IName).Open() as IWorkspace;
			for (int i = 0; i < imap_0.LayerCount; i++)
			{
				...
				if (layer.Visible)
				{
					if (layer is IGroupLayer)
					{
						Clip.ExtractSpecifyHRegFeatures(workspace as IFeatureWorkspace, layer as ICompositeLayer, igeometry_0);
					}
					if (layer is IFeatureLayer)
					{
						try
						...
						if (!igeometry_0.IsEmpty)
						{
							Clip.ExtractSelectFeatureFormFeatureCursor(layer.Name, Clip.searchFeatureFormLayer(layer, igeometry_0), (IFeatureWorkspace)workspace, igeometry_0);
						}
					}
				}
			}
		...
		public static void ExtractSpecifyHRegFeatures(IWorkspaceName iworkspaceName_0, IMap imap_0, IGeometry igeometry_0, IMap imap_1)
		{
			(new SelectionEnvironment()).CombinationMethod = esriSelectionResultEnum.esriSelectionResultNew;
			IWorkspace workspace = (iworkspaceName_0 as IName).Open() as IWorkspace;
			...
			for (int j = featureLayers.Count - 1; j >= 0; j--)
			{
				...
				if (!igeometry_0.IsEmpty)
				{
					Clip.ExtractSelectFeatureFormFeatureCursor(item, Clip.searchFeatureFormLayer(item, igeometry_0), (IFeatureWorkspace)workspace, igeometry_0, imap_1);
				}
			}
			Marshal.ReleaseComObject(workspace);
			workspace = null;
		...
		private static void InsertFeature(IFeatureCursor ifeatureCursor_0, IFeatureBuffer ifeatureBuffer_0, IFields ifields_0, IFeature ifeature_0, IGeometry igeometry_0, IGeometry igeometry_1)
		{
			...
			for (int i = 0; i < field.FieldCount; i++)
			{
				...
				if ((field1.Type == esriFieldType.esriFieldTypeGeometry ? false : field1.Type != esriFieldType.esriFieldTypeOID))
				{
