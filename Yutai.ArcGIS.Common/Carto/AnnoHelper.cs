	...
	public class AnnoHelper
	{
		...
		public static void ConvertLabels2Anno(IMap imap_0, ILayer ilayer_0, string string_0, double double_0)
		{
			...
			if (ilayer0 != null)
			{
				...
				{
					annotationProperties.QueryItem(i, out annotateLayerProperty, out elementCollection, out elementCollection1);
					if (annotateLayerProperty != null)
					{
						annotateLayerPropertiesCollectionClass.Add((annotateLayerProperty as IClone).Clone() as IAnnotateLayerProperties);
					}
				}
				annotateLayerProperty = null;
				IOverposterProperties overposterProperties = (imap_0 as IMapOverposter).OverposterProperties;
				IGraphicsLayer double0 = (imap_0.BasicGraphicsLayer as ICompositeGraphicsLayer).AddLayer(string_0, ilayer0);
				...
				{
					annotateLayerPropertiesCollectionClass.QueryItem(i, out annotateLayerProperty, out elementCollection, out elementCollection1);
					if (annotateLayerProperty != null)
					{
						...
						((annotateLayerProperty as ILabelEngineLayerProperties2).OverposterLayerProperties as IOverposterLayerProperties2).TagUnplaced = true;
					}
				}
				...
				for (i = 0; i < annotateLayerPropertiesCollectionClass.Count; i++)
				{
					annotateLayerPropertiesCollectionClass.QueryItem(i, out annotateLayerProperty, out elementCollection, out elementCollection2);
					if (annotateLayerProperty != null)
					{
				...
				if (workspace.Type != esriWorkspaceType.esriFileSystemWorkspace)
				{
					...
					for (i = 0; i < annotationProperties.Count; i++)
					{
						...
						if (annotateLayerProperty != null)
						{
							...
							symbolCollectionClass.AddSymbol(symbol.Clone() as ISymbol, string.Concat(annotateLayerProperty.Class, " ", i.ToString()), out symbolIdentifier2);
					...
					IOverposterProperties overposterProperties = (imap_0 as IMapOverposter).OverposterProperties;
					IAnnotationLayer annotationLayer = fDOGraphicsLayerFactoryClass.CreateAnnotationLayer(workspace as IFeatureWorkspace, featureClass.FeatureDataset, string_0, geometryDef, null, annotateLayerPropertiesCollectionClass, graphicsLayerScaleClass, symbolCollectionClass as ISymbolCollection, true, true, false, true, overposterProperties, "");
		...
		public static ITextElement MakeTextElement(string string_0, double double_0, double double_1, double double_2, int int_0)
		{
