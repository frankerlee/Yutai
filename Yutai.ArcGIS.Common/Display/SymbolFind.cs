...
namespace Yutai.ArcGIS.Common.Display
{
	public class SymbolFind
	{
		...
		public static IStyleGalleryItem FindStyleGalleryItem(string string_0, string string_1, string string_2, string string_3)
		{
			...
			try
			{
				if (File.Exists(string_1))
				{
					...
					else
					{
						...
						{
							if (Path.GetExtension((myStyleGallery as IStyleGalleryStorage).File[i]).ToLower() != ".serverstyle")
							{
								(myStyleGallery as IStyleGalleryStorage).RemoveFile((myStyleGallery as IStyleGalleryStorage).File[i]);
							}
						}
					}
					(myStyleGallery as IStyleGalleryStorage).AddFile(string_1);
					IStyleGalleryItem styleGalleryItem1 = SymbolFind.FindStyleGalleryItem(string_0, myStyleGallery, string_1, string_2, string_3);
					myStyleGallery = null;
					GC.Collect();
