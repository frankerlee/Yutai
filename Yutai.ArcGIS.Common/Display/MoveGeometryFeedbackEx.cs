			...
			if (!(iedgeFeature_0 is IComplexEdgeFeature))
			{
				...
				if (fromJunctionFeature is ISimpleJunctionFeature)
				{
					...
					for (i = 0; i < edgeFeatureCount; i++)
					{
						...
						if ((edgeFeature as IFeature).OID != oID && !this.iset_0.Find(edgeFeature) && !this.iset_1.Find(edgeFeature))
						{
							...
							if (!(edgeFeature is ISimpleEdgeFeature))
							{
								...
								num2 = -1;
								flag = false;
								if (hitTest.HitTest(shape, 0, esriGeometryHitPartType.esriGeometryPartVertex, pointClass, ref num, ref num1, ref num2, ref flag))
								{
							...
							else
							{
								...
								{
									shape = (shapeCopy as IPointCollection).Point[(shapeCopy as IPointCollection).PointCount - 1];
									lineMovePointFeedbackClass.Start(shapeCopy as IPolyline, (shapeCopy as IPointCollection).PointCount - 1, shape);
								}
								else
								{
					...
					for (i = 0; i < edgeFeatureCount; i++)
					{
						...
						if ((edgeFeature as IFeature).OID != oID && !this.iset_0.Find(edgeFeature) && !this.iset_1.Find(edgeFeature))
						{
							...
							if (!(edgeFeature is ISimpleEdgeFeature))
							{
								...
								num2 = -1;
								flag = false;
								if (hitTest.HitTest(point, 0, esriGeometryHitPartType.esriGeometryPartVertex, pointClass, ref num, ref num1, ref num2, ref flag))
								{
									lineMovePointFeedbackClass = new LineMovePointFeedback()
									{
										Display = this.iscreenDisplay_0
					...
					for (int j = 0; j < edgeFeatureCount; j++)
					{
						...
						if ((edgeFeature as IFeature).OID != oID && !this.iset_0.Find(edgeFeature) && !this.iset_1.Find(edgeFeature))
						{
							...
							if (!(edgeFeature is ISimpleEdgeFeature))
							{
								...
								int num2 = -1;
								bool flag = false;
								if (shape.HitTest(point, 0, esriGeometryHitPartType.esriGeometryPartVertex, pointClass, ref num, ref num1, ref num2, ref flag))
								{
									lineMovePointFeedbackClass = new LineMovePointFeedback()
									{
										Display = this.iscreenDisplay_0
							...
							else
							{
								...
								{
									point1 = (shapeCopy as IPointCollection).Point[(shapeCopy as IPointCollection).PointCount - 1];
									lineMovePointFeedbackClass.Start(shapeCopy as IPolyline, (shapeCopy as IPointCollection).PointCount - 1, point1);
								}
								else
								{
				...
				for (int i = 0; i < edgeFeatureCount; i++)
				{
					...
					if (!this.iset_0.Find(edgeFeature) && !this.iset_1.Find(edgeFeature))
					{
						...
						else
						{
							if (edgeFeature.FromJunctionEID != eID)
							{
								point = (shapeCopy as IPointCollection).Point[(shapeCopy as IPointCollection).PointCount - 1];
								lineMovePointFeedbackClass.Start(shapeCopy as IPolyline, (shapeCopy as IPointCollection).PointCount - 1, point);
							}
							else
							{
