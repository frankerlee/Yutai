...
namespace Yutai.ArcGIS.Common.Editor
{
	public class Editor
	{
		...
		public static YTEditTemplate CurrentEditTemplate
		{
			get
			{
				return Editor.m_CurrentEditTemplate;
			}
			set
			{
		...
		public static IFeatureLayer CurrentLayer
		{
			get
			{
				return Editor.m_CurrentSkectchLayer;
			}
			set
			{
		...
		public static bool DrawNode
		{
			get
			{
				return Editor._DrawNode;
			}
			set
			{
				Editor._DrawNode = value;
			}
		}
		...
		public static IWorkspaceEdit EditWorkspace
		{
			get
			{
				return Editor.m_pEditWorkspace;
			}
			set
			{
				Editor.m_pEditWorkspace = value;
			}
		}
		...
		public static bool EnableSketch
		{
			get
			{
				return Editor.m_EnableSketch;
			}
			set
			{
				Editor.m_EnableSketch = value;
			}
		}
		...
		public static bool EnableUndoRedo
		{
			get
			{
				return Editor.m_EnableUndoRedo;
			}
			set
			{
				Editor.m_EnableUndoRedo = value;
			}
		}
		...
		public static bool IsAdvanceControlTool
		{
			get
			{
				return Editor.m_IsAdvanceControlTool;
			}
		}
		...
		public static bool IsCheckRegisterVeision
		{
			get
			{
				return Editor.m_IsCheckRegisterVeision;
			}
			set
			{
				Editor.m_IsCheckRegisterVeision = value;
			}
		}

		public static bool IsRemoveUnEditLayerSelect
		{
			get;
			set;
		}

		public static bool IsSnapEndPoint
		{
			get;
			set;
		}

		public static bool IsSnapLine
		{
			get;
			set;
		}

		public static bool IsSnapPoint
		{
			get;
			set;
		}

		public static bool IsSnapVertexPoint
		{
			get;
			set;
		}

		public static IWorkspaceEdit SecondEditWorkspace
		{
			get;
			set;
		}
		...
		private static void CheckGroupLayerEdit(IArray pArray, ICompositeLayer icompositeLayer_0)
		{
			for (int i = 0; i < icompositeLayer_0.Count; i++)
			{
				...
				else if (layer is IFeatureLayer)
				{
					...
					if (featureClass != null && !(featureClass is ICoverageFeatureClass) && Editor.LayerIsHasEditprivilege(layer as IFeatureLayer))
					{
				...
				if (Editor.EditWorkspace != null)
				{
					...
					if (workspace.ConnectionProperties.IsEqual((Editor.EditWorkspace as IWorkspace).ConnectionProperties))
					{
						...
						else if (workspace is IVersionedWorkspace && (idataset_0 as IVersionedObject).IsRegisteredAsVersioned)
						{
							...
							{
								if (!Editor.m_SysGrants.GetStaffAndRolesLayerPri(AppConfigInfo.UserID, 2, idataset_0.Name))
								{
									goto Label1;
								}
								flag = true;
			...
			if (!(ifeatureLayer_0.FeatureClass is ICoverageFeatureClass))
			{
				...
				if (workspace != null)
				{
					...
					if (workspace.IsBeingEdited())
					{
						...
						else if (workspace is IVersionedWorkspace && (ifeatureLayer_0.FeatureClass as IVersionedObject).IsRegisteredAsVersioned)
						{
							if (!(AppConfigInfo.UserID.Length == 0 ? false : !(AppConfigInfo.UserID.ToLower() == "admin")))
							{
								flag1 = true;
							}
							else if (Editor.m_SysGrants.GetStaffAndRolesLayerPri(AppConfigInfo.UserID, 2, (ifeatureLayer_0.FeatureClass as IDataset).Name))
							{
								flag1 = true;
							}
						}
		...
		public static bool CheckLayerCanEdit(IFeatureClass ifeatureClass_0)
		{
			...
			else if (!(ifeatureClass_0 is ICoverageFeatureClass))
			{
				...
				if (workspace.ConnectionProperties.IsEqual((Editor.EditWorkspace as IWorkspace).ConnectionProperties))
				{
					...
					else if (workspace is IVersionedWorkspace && (ifeatureClass_0 as IVersionedObject).IsRegisteredAsVersioned)
					{
						if ((AppConfigInfo.UserID.Length == 0 ? false : !(AppConfigInfo.UserID.ToLower() == "admin")))
						{
							if (!Editor.SysGrants.GetStaffAndRolesLayerPri(AppConfigInfo.UserID, 2, (ifeatureClass_0 as IDataset).Name))
							{
								goto Label1;
							}
							flag = true;
		...
		private static int CheckStartEditing(ILayer ilayer_0, ref string string_0)
		{
			...
			else if (ilayer_0 is IFeatureLayer)
			{
				...
				if ((featureClass.Type == esriDatasetType.esriDTFeatureClass ? false : featureClass.Type != esriDatasetType.esriDTFeatureDataset))
				{
		...
		private static bool CheckStopEdits(ILayer ilayer_0, bool bool_0)
		{
			...
			try
			{
				...
				else if (ilayer_0 is IFeatureLayer)
				{
					...
					{
						IDataset featureClass = (IDataset)featureLayer.FeatureClass;
						if ((featureClass.Type == esriDatasetType.esriDTFeatureClass ? true : featureClass.Type == esriDatasetType.esriDTFeatureDataset))
						{
							IWorkspaceEdit workspace = (IWorkspaceEdit)featureClass.Workspace;
							if (workspace.IsBeingEdited())
							{
		...
		public static bool ConstructJunction(IMap imap_0, IFeature ifeature_0, IPoint pPoint, int int_0, int int_1, IFeature ifeature_1, IPoint ipoint_1, int int_2, int int_3)
		{
			bool flag;
			IPoint point;
			IPoint point1;
			...
			if (UtilityLicenseProviderCheck.Check())
			{
				...
				if ((geometry as IPointCollection).PointCount == 2)
				{
					...
					if ((pPoint as IProximityOperator).ReturnDistance(point) <= (pPoint as IProximityOperator).ReturnDistance(point1))
					{
				...
				if ((segmentCollection as IPointCollection).PointCount == 2)
				{
					...
					if ((ipoint_1 as IProximityOperator).ReturnDistance(point) <= (ipoint_1 as IProximityOperator).ReturnDistance(point1))
					{
				...
				IPoint point2 = Editor.CalculateJunction(segment.FromPoint, segment.ToPoint, segment1.FromPoint, segment1.ToPoint);
				if (point2 == null)
				{
					flag = false;
				}
		...
		public static void CopyRow(IRow irow_0, IRow irow_1)
		{
			...
			for (int i = 0; i < fields.FieldCount; i++)
			{
				...
				if ((field.Type == esriFieldType.esriFieldTypeOID || !field.Editable ? false : field.Type != esriFieldType.esriFieldTypeGeometry))
				{
					try
					{
		...
		public static void CopyRowEx(IRow irow_0, IRow irow_1)
		{
			...
			{
				LayerMap layerMap = CopyEnvironment.CopyMaps.GetLayerMap((irow_0.Table as IDataset).Name, (irow_1.Table as IDataset).Name);
				if (layerMap == null)
				{
					goto Label1;
				}
		...
		Label1:
			...
			for (int i = 0; i < fields.FieldCount; i++)
			{
				...
				if ((field1.Type == esriFieldType.esriFieldTypeOID || field1.Type == esriFieldType.esriFieldTypeGeometry || field1.Type == esriFieldType.esriFieldTypeGUID || field1.Type == esriFieldType.esriFieldTypeGUID || field1.Type == esriFieldType.esriFieldTypeRaster || field1.Type == esriFieldType.esriFieldTypeBlob ? false : field1.Editable))
				{
		...
		public static void CopySelectFeatureToTargetFeatureLayer(IFeatureLayer ifeatureLayer_0, IFeatureLayer ifeatureLayer_1)
		{
		}
					...
					if (polygonClass != null)
					{
						try
						{
							...
							else
							{
								...
								CreateFeatureTool.CreateFeature(polygonClass as IGeometry, imap_0 as IActiveView, ifeatureLayer_0);
							}
						}
						catch (Exception exception)
						{
					...
					if (polylineClass != null)
					{
						try
						{
							...
							else
							{
								...
								CreateFeatureTool.CreateFeature(polylineClass as IGeometry, imap_0 as IActiveView, ifeatureLayer_0);
							}
						}
						catch (Exception exception)
						{
		...
		public static bool DeleteFeature(IFeature ifeature_0, ref bool bool_0)
		{
			...
			if ((@class.Type == esriDatasetType.esriDTFeatureClass ? false : @class.Type != esriDatasetType.esriDTFeatureDataset))
			{
				flag1 = false;
			}
			else
		...
		public static void ExportGeometryCollection(StreamWriter streamWriter_0, IGeometryCollection igeometryCollection_0)
		{
			for (int i = 0; i < igeometryCollection_0.GeometryCount; i++)
			{
		...
		public static void ExportGeometryToText(string string_0, IMap imap_0)
		{
			...
			while (layer != null)
			{
				...
				{
					if ((featureLayer.FeatureClass.ShapeType == esriGeometryType.esriGeometryPolygon ? true : featureLayer.FeatureClass.ShapeType == esriGeometryType.esriGeometryPolyline) && (featureLayer as IFeatureSelection).SelectionSet.Count > 0)
					{
						string string0 = string_0;
						if (num != 0)
						{
						...
						if (shape != null)
						{
							...
							if (!(polylineClass as IGeometry).IsEmpty)
							{
								if (((feature.Class as IDataset).Workspace as IWorkspaceEdit).IsBeingEdited())
								{
									...
									int num1 = -1;
									int num2 = -1;
									if (!GeometryOperator.TestGeometryHit(mapUnits, point, shape, out pointClass, ref num, ref num1, ref num2, out flag))
									{
										point = (polylineClass as IPointCollection).Point[(polylineClass as IPointCollection).PointCount - 1];
										GeometryOperator.TestGeometryHit(mapUnits, point, shape, out pointClass, ref num, ref num1, ref num2, out flag);
										if (!flag)
										{
											pointClass.Z = 0;
		...
		public static void FeatureAttributreCopy(IFeature ifeature_0, IFeature ifeature_1)
		{
			...
			{
				IField field1 = fields.Field[i];
				if ((!(field1.Name != shapeFieldName) || field1.Type == esriFieldType.esriFieldTypeOID ? false : field1.Editable))
				{
		...
		public static void GetClosesFeature(IMap imap_0, IPoint pPoint, double pDistance, esriGeometryType esriGeometryType_0, out IFeature ifeature_0)
		{
			IEnvelope envelope = pPoint.Envelope;
			...
			while (layer != null)
			{
				if (layer is IFeatureLayer)
				{
					...
					else if (!(esriGeometryType_0 == esriGeometryType.esriGeometryAny ? true : esriGeometryType_0 == (layer as IFeatureLayer).FeatureClass.ShapeType))
					{
						layer = layers.Next();
						continue;
					}
					...
					{
						IFeatureCursor featureCursor = (layer as IFeatureLayer).FeatureClass.Search(spatialFilterClass, false);
						for (IFeature i = featureCursor.NextFeature(); i != null; i = featureCursor.NextFeature())
						{
		...
		public static void GetClosestSelectedFeature(IFeatureCache ifeatureCache_0, IPoint pPoint, out IFeature ifeature_0)
		{
		...
		private static double GetClosestSelectedFeature(IPoint pPoint, IFeatureCursor ifeatureCursor_0, ref IFeature ifeature_0)
		{
			double num;
			IProximityOperator ipoint0 = (IProximityOperator)pPoint;
		...
		public static IFeature GetHitFeature(IMap imap_0, IPoint pPoint, double pDistance, out IFeatureLayer ifeatureLayer_0)
		{
		...
		public static IFeature GetHitLineFeature(IMap imap_0, IPoint pPoint, double pDistance, out IFeatureLayer ifeatureLayer_0)
		{
			ifeatureLayer_0 = null;
			IFeature feature = null;
			double num = 0;
		...
		public static IFeature GetHitLineOrAreaFeature(IMap imap_0, IPoint pPoint, double pDistance, out IFeatureLayer ifeatureLayer_0)
		{
			ifeatureLayer_0 = null;
			IFeature feature = null;
			double num = 0;
			...
			for (int i = 0; i < imap_0.LayerCount; i++)
			{
				...
				if (layer != null && layer.Visible)
				{
					if ((layer.FeatureClass.ShapeType == esriGeometryType.esriGeometryPolyline ? true : layer.FeatureClass.ShapeType == esriGeometryType.esriGeometryPolygon))
					{
						spatialFilterClass.GeometryField = layer.FeatureClass.ShapeFieldName;
						IFeatureCursor featureCursor = layer.Search(spatialFilterClass, false);
						IFeature feature1 = null;
		...
		public static bool LayerCanEdit(IFeatureLayer ifeatureLayer_0)
		{
			...
			if (Editor.EditWorkspace != null)
			{
				...
				else if (!(featureClass is ICoverageFeatureClass))
				{
					try
					{
						if (ApplicationRef.AppContext.Config.IsSupportZD && ZD.ZDRegister.IsZDFeatureClass(ifeatureLayer_0.FeatureClass))
						{
							flag = false;
							return flag;
						}
		...
		public static IPointCollection LineIntersect(IPolyline ipolyline_0, IEnvelope ienvelope_0)
		{
			...
			{
				IGeometry geometry = ipolyline0.Intersect(polylineClass as IGeometry, esriGeometryDimension.esriGeometry0Dimension);
		...
		public static void RefreshLayerWithSelection(IMap imap_0, IEnvelope ienvelope_0, esriViewDrawPhase esriViewDrawPhase_0)
		{
			if (imap_0 != null)
			{
		...
		public static bool Snap2Point(IPoint pPoint, IPoint ipoint_1, IPoint ipoint_2, ISnapEnvironment isnapEnvironment_0, IActiveView iactiveView_0, ISymbol isymbol_0, ref IAnchorPoint ianchorPoint_0)
		{
			bool flag = false;
			...
			if (Editor.UseSnap)
			{
				...
				Label1:
					...
					if (Editor.IsSnapEndPoint)
					{
						for (i = 0; i < featureCacheClass.Count; i++)
						{
							...
							{
								pointClass = new ESRI.ArcGIS.Geometry.Point();
								if (((IHitTest)feature.Shape).HitTest(pPoint, pDistance, esriGeometryHitPartType.esriGeometryPartEndpoint, pointClass, ref num2, ref num3, ref num4, ref flag1))
								{
									if (num1 < 0)
									{
										num1 = num2;
				...
				Label2:
					if (Editor.IsSnapVertexPoint)
					{
						...
						{
							feature = featureCacheClass.Feature[i];
							if ((feature.Shape.GeometryType == esriGeometryType.esriGeometryPoint ? false : feature.Shape.GeometryType != esriGeometryType.esriGeometryMultipoint))
							{
								pointClass = new ESRI.ArcGIS.Geometry.Point();
								if (((IHitTest)feature.Shape).HitTest(pPoint, pDistance, esriGeometryHitPartType.esriGeometryPartVertex, pointClass, ref num2, ref num3, ref num4, ref flag1))
								{
									if (num1 < 0)
									{
										num1 = num2;
		...
		private static void SplitGeometry1(IFeature ifeature_0, int int_0, int int_1, int int_2, int int_3)
		{
			...
			else
			{
				...
				if (int_3 - int_1 + 1 != geometry.PointCount)
				{
					...
					for (i = 0; i < int_0 - 1; i++)
					{
						polygonClass.AddGeometry((shapeCopy.Geometry[i] as IClone).Clone() as IGeometry, ref value, ref value);
					}
					IPointCollection ringClass = null;
					if (int_1 != 0)
					{
		...
		public static bool StartEditing(IMap imap_0)
		{
			...
			if (arrayClass.Count != 0)
			{
				...
				else
				{
					...
					{
						MessageBox.Show("不能编辑任何图层，请检查是否加载了要素图层，加载的要素图层是否是否已经进行了版本注册或是否有更新权限！", "开始编辑", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
						editWorkspace = false;
						return editWorkspace;
					}
		...
		public static bool StopEditing()
		{
			...
			if (Editor.EditWorkspace != null)
			{
				...
				{
					DialogResult dialogResult = MessageBox.Show("数据已经被修改过，保存修改吗?", "更改提示", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
		...
		public static bool StopEditing(IDataset idataset_0, bool bool_0, bool bool_1)
		{
			...
			else
			{
				...
				dialogResult = (flag2 ? MessageBox.Show("数据已经被修改过，保存修改吗?", "更改提示", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) : DialogResult.No);
				if (dialogResult != DialogResult.Cancel)
				{
        ...
        public static bool StopEditing(IMap imap_0, bool bool_0, bool bool_1)
        {
            ...
            else
            {
                ...
                {
                    yes = MessageBox.Show("数据已经被修改过，保存修改吗?", "更改提示", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
                }
        ...
        public static IPolyline TrimPolyLine(IMap imap_0, IGeometry igeometry_0, IEnvelope ienvelope_0, double double_0)
        {
            if ((igeometry_0 as IGeometryCollection).GeometryCount <= 1)
            {
                ...
                if (points2.PointCount > 0)
                {
                    ...
                    int num11 = -1;
                    int num12 = -1;
                    if (!GeometryOperator.TestGeometryHit(double_0, point, polycurve, out point2, ref num, ref num2, ref num3, out flag))
                    {
                        return null;
                    }
                    for (num13 = 0; num13 < points2.PointCount; num13++)
                    {
                        ...
                        GeometryOperator.TestGeometryHit(double_0, point3, polycurve, out point2, ref num4, ref num5, ref num6, out flag2);
                        if (num5 > num2)
                        {
                            if (num11 == -1)
                            {
                    ...
                    if (num7 == -1)
                    {
                        ...
                        GeometryOperator.TestGeometryHit(double_0 / 10.0, point3, igeometry_0, out point4, ref num, ref num2, ref num3, out flag);
                        ...
                        {
                            geometrys.AddGeometry((igeometry_0 as IGeometryCollection).get_Geometry(num13), ref before, ref before);
                        }
        ...
        public static IPolyline TrimPolyLine(IMap imap_0, IGeometry igeometry_0, IPoint ipoint_0, double double_0)
        {
            if ((igeometry_0 as IGeometryCollection).GeometryCount <= 1)
            {
                ...
                if (points.PointCount > 0)
                {
                    ...
                    int num11 = -1;
                    int num12 = -1;
                    if (!GeometryOperator.TestGeometryHit(double_0, ipoint_0, polycurve, out point, ref num, ref num2, ref num3, out flag))
                    {
                        return null;
                    }
                    for (num13 = 0; num13 < points.PointCount; num13++)
                    {
                        ...
                        GeometryOperator.TestGeometryHit(double_0, point2, polycurve, out point, ref num4, ref num5, ref num6, out flag2);
                        if (num5 > num2)
                        {
                            if (num11 == -1)
                            {
                    ...
                    if (num7 == -1)
                    {
                        ...
                        GeometryOperator.TestGeometryHit(double_0 / 10.0, point2, igeometry_0, out point3, ref num, ref num2, ref num3, out flag);
