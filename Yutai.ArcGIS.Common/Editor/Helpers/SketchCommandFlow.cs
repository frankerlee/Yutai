	...
	public class SketchCommandFlow : ICommandFlow
	{
		...
		public bool HandleCommand(string string_2)
		{
			...
			else if (this.int_0 != 0)
			{
				if (this.icommandFlow_0 != null)
				{
					...
					if ((this.icommandFlow_0 is ThreePointArcCommandFlow ? true : this.icommandFlow_0 is ConstructTangentArcCommandFlow))
					{
							...
							if (lower != null)
							{
								switch (lower)
								{
									...
									case "t":
									{
										if (SketchToolAssist.Feedback is INewPolylineFeedback)
										{
											(SketchToolAssist.Feedback as INewPolylineFeedback).ChangeLineType(enumLineType.LTTangentCircularArc);
										}
										if (SketchToolAssist.Feedback is INewPolygonFeedbackEx)
										{
											(SketchToolAssist.Feedback as INewPolygonFeedbackEx).ChangeLineType(enumLineType.LTTangentCircularArc);
										}
									...
									case "d":
									{
										...
										pointClass = (SketchToolAssist.Feedback as INewPolylineFeedback).ReverseOrientation();
										if (pointClass == null)
										{
											break;
										}
									...
									case "f":
									{
										SketchShareEx.EndSketch(false, this._appContext.MapControl.Map as IActiveView, Editor.CurrentEditTemplate.FeatureLayer);
										this.int_0 = 0;
			...
			else
			{
				...
				try
				{
					...
					else
					{
						...
						SketchShareEx.SketchMouseDown(pointClass, this._appContext.MapControl.Map as IActiveView, Editor.CurrentEditTemplate.FeatureLayer);
						if ((Editor.CurrentEditTemplate.FeatureLayer.FeatureClass.ShapeType == esriGeometryType.esriGeometryPoint ? false : Editor.CurrentEditTemplate.FeatureLayer.FeatureClass.FeatureType != esriFeatureType.esriFTAnnotation))
						{
		...
		public void ShowCommandLine()
		{
			...
			switch (this.int_0)
			{
				case 0:
				{
					if ((shapeType == esriGeometryType.esriGeometryPoint ? false : Editor.CurrentEditTemplate.FeatureLayer.FeatureClass.FeatureType != esriFeatureType.esriFTAnnotation))
					{
				...
				case 2:
				{
					this._appContext.ShowCommandString("下一点或[撤销(U)/重做(R)/三点弧(A)/切线弧(T)/隔一点(J)/隔点闭合(G)/结束(F)/线反向(D)]:", CommandTipsType.CTTCommandTip);
					break;
				}
				default:
				{
