...
{
	public class TrimCommandFlow : ICommandFlow
	{
						...
						if (string_0.Length != 0)
						{
							...
							else
							{
								...
								else
								{
									...
									IAppContext iapplication0 = this._appContext;
									selectionCount = this._appContext.MapControl.Map.SelectionCount;
									iapplication0.ShowCommandString(string.Concat("找到", selectionCount.ToString(), "个对象"), CommandTipsType.CTTLog);
									this.ShowCommandLine();
								}
							}
						}
					...
					else if (this.int_0 == 1)
					{
						...
						if ((int)strArrays.Length >= 2)
						{
							...
							if (this.idisplayFeedback_0 == null)
							{
								...
								{
									IPolyline polyline = Editor.TrimPolyLine(this._appContext.MapControl.Map, hitLineFeature.Shape, pointClass, mapUnits);
							...
							else
							{
								...
								for (int i = 0; i < intersectsLineFeatures.Count; i++)
								{
									...
									if (workspaceEdit.IsBeingEdited())
									{
										...
										this.method_0(envelope, mapUnits, intersectsLineFeatures[i]);
									}
								}
								if (workspace != null)
								{
		...
		public void ShowCommandLine()
		{
			...
			switch (this.int_0)
			{
				...
				case 1:
				{
					this._appContext.ShowCommandString("请选择要修剪的对象:", CommandTipsType.CTTCommandTip);
					break;
				}
			}
		}
