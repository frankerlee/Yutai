...
{
    public class ConstructTangentArcCommandFlow : ICommandFlow
    {
        ...
        public ConstructTangentArcCommandFlow()
        {
        }
                ...
                try
                {
                    ...
                    if (string_1.Length != 0)
                    {
                        ...
                        if ((int) strArrays.Length < 2)
                        {
                            ...
                            //MessageService.Current.Warn("输入不正确");
                            flag = true;
                            return flag;
                        }
                        else
        ...
        public void ShowCommandLine()
        {
            ...
            {
                this._appContext.ShowCommandString(
                    "下一点或[闭合(C)/撤销(U)/重做(R)/三点弧(A)/切线弧(T)/隔一点(J)/隔点闭合(G)/结束(F)/线反向(D)]:", CommandTipsType.CTTCommandTip);
            }
        }
    }
}