...
namespace Yutai.ArcGIS.Common.Editor.Helpers
{
	public class IntersectionSnapAgent : IFeatureSnapAgent, IEngineSnapAgent
	{
		...
		public esriGeometryHitPartType HitType
		{
			get
			{
				return esriGeometryHitPartType.esriGeometryPartBoundary;
			}
			set
			{
			}
		...
		public bool Snap(IGeometry igeometry_0, IPoint ipoint_0, double double_0)
		{
			...
			else if (this.ifeatureCache_0.Count != 0)
			{
				...
				for (i = 0; i < this.ifeatureCache_0.Count; i++)
				{
					...
					if ((shape is IPolyline ? true : shape is IPolygon) && shape.HitTest(ipoint_0, double_0, esriGeometryHitPartType.esriGeometryPartBoundary, pointClass, ref num, ref num1, ref num2, ref flag2))
					{
				...
				for (i = 0; i < arrayClass.Count; i++)
				{
					...
					for (int j = 0; j < arrayClass.Count; j++)
					{
						if (i != j)
						{
							if (((IGeometry)arrayClass.Element[i]).GeometryType != ((IGeometry)arrayClass.Element[j]).GeometryType)
							{
							...
							{
								geometry = element.Intersect((IGeometry)arrayClass.Element[j], esriGeometryDimension.esriGeometry0Dimension);
								if (geometry != null)
								{
									if (geometry is IPointCollection)
									{
				...
				if (shape.HitTest(ipoint_0, double_0, esriGeometryHitPartType.esriGeometryPartVertex, pointClass, ref num, ref num1, ref num2, ref flag2))
				{
