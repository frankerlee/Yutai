...
namespace Yutai.ArcGIS.Common.Editor.Helpers
{
	public class ExtendLineCommandFlow : ICommandFlow
	{
					...
					else if (this.int_0 == 0)
					{
						...
						if (string_0.Length != 0)
						{
							...
							else
							{
								...
								{
									mapUnits = CommonHelper.ConvertPixelsToMapUnits(this._appContext.MapControl.Map as IActiveView, 6);
									hitLineFeature = Editor.GetHitLineFeature(this._appContext.MapControl.Map, pointClass, mapUnits, out featureLayer);
								...
								else
								{
                                    ...
                                    appContext.ShowCommandString(string.Concat("找到", selectionCount.ToString(), "个对象"), CommandTipsType.CTTLog);
									this.ShowCommandLine();
								}
							}
						}
					...
					else if (this.int_0 == 2)
					{
						...
						if ((int)strArrays.Length >= 2)
						{
							...
							if (this.idisplayFeedback_0 == null)
							{
								...
								{
									polyline = null;
									polyline = (this.int_1 != 0 ? Editor.ExtendPolyLineEx(this._appContext.MapControl.Map, hitLineFeature.Shape) : Editor.ExtendPolyLine(this._appContext.MapControl.Map, hitLineFeature.Shape));
							...
							else
							{
								...
								selectionCount = this._appContext.MapControl.Map.SelectionCount;
								application.ShowCommandString(string.Concat("找到", selectionCount.ToString(), "个对象"), CommandTipsType.CTTLog);
								mapUnits = CommonHelper.ConvertPixelsToMapUnits(this._appContext.MapControl.Map as IActiveView, 6);
								IList<IFeature> intersectsLineFeatures = Editor.GetIntersectsLineFeatures(this._appContext.MapControl.Map, envelope);
								workspace = null;
								for (int i = 0; i < intersectsLineFeatures.Count; i++)
								{
									IWorkspaceEdit workspaceEdit = (intersectsLineFeatures[i].Class as IDataset).Workspace as IWorkspaceEdit;
									if (workspaceEdit.IsBeingEdited())
									{
										...
										polyline = null;
										polyline = (this.int_1 != 0 ? Editor.ExtendPolyLineEx(this._appContext.MapControl.Map, intersectsLineFeatures[i].Shape) : Editor.ExtendPolyLine(this._appContext.MapControl.Map, intersectsLineFeatures[i].Shape));
										if (polyline != null && !polyline.IsEmpty && (polyline as IPointCollection).PointCount > 1)
										{
											(polyline as ITopologicalOperator).Simplify();
		...
		private void method_0(IEnvelope ienvelope_0, double double_2, IFeature ifeature_1)
		{
			try
			{
				IPolyline polyline = Editor.TrimPolyLine(this._appContext.MapControl.Map, ifeature_1.Shape, ienvelope_0, double_2);
				if (polyline != null && !polyline.IsEmpty && (polyline as IPointCollection).PointCount > 1)
				{
					(polyline as ITopologicalOperator).Simplify();
		...
		public void ShowCommandLine()
		{
			...
			switch (this.int_0)
			{
				...
				case 1:
				{
					this._appContext.ShowCommandString(string.Concat("生成交点[Y/N]<", (this.int_1 == 0 ? "N" : "Y"), ">:"), CommandTipsType.CTTCommandTip);
					break;
				}
				case 2:
				{
