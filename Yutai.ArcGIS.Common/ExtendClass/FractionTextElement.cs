...
namespace Yutai.ArcGIS.Common.ExtendClass
{
	...
	public class FractionTextElement : IBoundsProperties, IClone, IElement, IElementProperties, IElementProperties2, IGraphicElement, ITransform2D, IFractionTextElement, IPersistStream, IPersist
	{
		...
		public ITextSymbol DenominatorTextSymbol
		{
			get
			{
				return this.m_DenominatorTextSymbol;
			}
			set
			{
		...
		public ITextSymbol NumeratorTextSymbol
		{
			get
			{
				return this.m_NumeratorTextSymbol;
			}
			set
			{
		...
		public void QueryOutline(IDisplay Display, IPolygon Outline)
		{
			if (this.m_resetGeometry)
			{
				...
				IPoint pointClass = new ESRI.ArcGIS.Geometry.Point();
				pointClass.PutCoords((this.m_pGeometry.XMin + this.m_pGeometry.XMax) / 2, (this.m_pGeometry.YMax + this.m_pGeometry.YMin) / 2);
				(this.m_NumeratorTextSymbol as ISymbol).QueryBoundary(mPCachedDisplay, this.m_pCachedDisplay.DisplayTransformation, pointClass, Outline);
				double width = Outline.Envelope.Width;
				double height = Outline.Envelope.Height;
				(this.m_DenominatorTextSymbol as ISymbol).QueryBoundary(mPCachedDisplay, this.m_pCachedDisplay.DisplayTransformation, pointClass, Outline);
				...
				this.m_pGeometry = new Envelope() as IEnvelope;
				this.m_pGeometry.PutCoords(pointClass.X - num1 / 2, pointClass.X + num1 / 2, pointClass.Y - num2 / 2, pointClass.Y + num2 / 2);
