...
namespace Yutai.ArcGIS.Common.ExtendClass
{
	...
	public sealed class TriangleElementClass : ITriangleElement, IElement, IElementProperties, IElementProperties2, IElementProperties3, IBoundsProperties, ITransform2D, IGraphicElement, IPersistVariant, IClone, IDocumentVersionSupportGEN
	{
		...
		public esriAnchorPointEnum AnchorPoint
		{
			get
			{
				return this.m_anchorPointType;
			}
			set
			{
				this.m_anchorPointType = value;
			}
		}
		...
		public bool AutoTransform
		{
			get
			{
				return this.m_autoTrans;
			}
			set
			{
				this.m_autoTrans = value;
			}
		}
		...
		public object CustomProperty
		{
			get
			{
				return this.m_customProperty;
			}
			set
			{
				this.m_customProperty = value;
			}
		}
		...
		public IGeometry Geometry
		{
			get
			{
				return this.Clone(this.m_pointGeometry) as IGeometry;
			}
			set
			{
				try
		...
		public double ReferenceScale
		{
			get
			{
				return this.m_scaleRef;
			}
			set
			{
				this.m_scaleRef = value;
			}
		}
		...
		public ISelectionTracker SelectionTracker
		{
			get
			{
				return this.m_selectionTracker;
			}
		}
		...
		public bool IsEqual(IClone other)
		{
			...
			return ((!(triangleElementClass.Name == this.m_elementName) || !(triangleElementClass.Type == this.m_elementType) || triangleElementClass.AutoTransform != this.m_autoTrans || triangleElementClass.ReferenceScale != this.m_scaleRef || triangleElementClass.Angle != this.m_rotation || triangleElementClass.Size != this.m_size || triangleElementClass.AnchorPoint != this.m_anchorPointType || !((IClone)triangleElementClass.Geometry).IsEqual((IClone)this.m_triangle) || !((IClone)triangleElementClass.FillSymbol).IsEqual((IClone)this.m_fillSymbol) ? true : !((IClone)triangleElementClass.SpatialReference).IsEqual((IClone)this.m_nativeSR)) ? false : true);
		}
		...
		public void QueryBounds(IDisplay Display, IEnvelope Bounds)
		{
			...
			((ISymbol)this.m_fillSymbol).QueryBoundary(Display.hDC, Display.DisplayTransformation, this.m_triangle, polygonClass);
		...
		private void SetupDeviceRatio(int hDC, IDisplay display)
		{
			if (display.DisplayTransformation != null)
			{
				if (display.DisplayTransformation.Resolution != 0)
				{
					...
					{
						this.m_dDeviceRatio = this.m_dDeviceRatio * display.DisplayTransformation.ReferenceScale / display.DisplayTransformation.ScaleRatio;
					}
				}
			}
		...
		{
			if ((this.m_cachedDisplay == null || this.m_nativeSR == null || this.m_triangle == null ? false : null != this.m_cachedDisplay.DisplayTransformation.SpatialReference))
			{
