...
namespace Yutai.ArcGIS.Common.ExtendClass
{
	...
	public sealed class LabelLineElementClass : ILineElement, IElement, IElementProperties, IElementProperties2, IElementProperties3, IBoundsProperties, ITransform2D, IGraphicElement, IPersistVariant, IClone, IDocumentVersionSupportGEN
	{
		...
		public IGeometry Geometry
		{
			get
			{
				return this.Clone(this.m_triangle) as IGeometry;
			}
			set
			{
				try
		...
		public bool IsEqual(IClone other)
		{
			...
			return ((!(labelLineElementClass.Name == this.m_elementName) || !(labelLineElementClass.Type == this.m_elementType) || labelLineElementClass.AutoTransform != this.m_autoTrans || labelLineElementClass.ReferenceScale != this.m_scaleRef || labelLineElementClass.AnchorPoint != this.m_anchorPointType || !((IClone)labelLineElementClass.Geometry).IsEqual((IClone)this.m_triangle) || !((IClone)labelLineElementClass.Symbol).IsEqual((IClone)this.m_fillSymbol) ? true : !((IClone)labelLineElementClass.SpatialReference).IsEqual((IClone)this.m_nativeSR)) ? false : true);
		}
