...
namespace Yutai.ArcGIS.Common.SymbolEx
{
	...
	public class LabelSymbol : ISymbol, ILineSymbol, IDisplayName, IClone, IPersistVariant, ISymbolRotation, IMapLevel, IPropertySupport, ILabelSymbol
	{
		...
		private void Draw(Graphics g, Utility.POINTAPI[] ctrlpts)
		{
			...
			for (i = 0; i < (int)ctrlpts.Length - 1; i++)
			{
				...
				double num6 = LabelSymbol.azimuth((double)ctrlpts[i].x, (double)ctrlpts[i].y, (double)ctrlpts[i + 1].x, (double)ctrlpts[i + 1].y);
			...
			double num10 = LabelSymbol.azimuth((double)ctrlpts[length - 2].x, (double)ctrlpts[length - 2].y, (double)ctrlpts[length - 1].x, (double)ctrlpts[length - 1].y);
			double num11 = 30;
			...
			g.DrawLine(Pens.Black, (float)ctrlpts[length - 1].x, (float)ctrlpts[length - 1].y, (float)num15, (float)num16);
			g.DrawLine(Pens.Black, (float)ctrlpts[length - 1].x, (float)ctrlpts[length - 1].y, (float)num17, (float)num18);
			if (length <= 2)
			{
		...
		private void Draw2(Graphics g, Utility.POINTAPI[] ctrlpts)
		{
			...
			{
				num2 = item / num7;
				num3 = LabelSymbol.azimuth((double)ctrlpts[i].x, (double)ctrlpts[i].y, (double)ctrlpts[i + 1].x, (double)ctrlpts[i + 1].y);
				num4 = num3 + 270;
		...
		private void QueryBound(IPointCollection pts, ITransformation displayTransform, out double minx, out double miny, out double maxx, out double maxy)
		{
			IPoint point;
			IPoint point1;
			int i;
		...
		private void QueryBoundsFromGeom(int hDC, ref IDisplayTransformation transform, ref IPolygon boundary, ref IPointCollection points)
		{
			double map = 0;
			double num = 0;
			double map1 = 0;
