...
namespace Yutai.ArcGIS.Common.SymbolEx
{
	...
	public class MyText : ISymbol, ITextSymbol, IDisplayName, IClone, IPersistVariant, ISymbolRotation, IMapLevel, ITextParserSupport, IMask, IQueryGeometry, IMarginProperties, ICharacterOrientation, IPropertySupport, IDocumentVersionSupportGEN, IMyText
	{
		...
		bool ESRI.ArcGIS.esriSystem.IClone.IsEqual(IClone other)
		{
			...
			else
			{
				...
				else if (myText1.DenominatorText == myText.DenominatorText)
				{
					...
					else if (!ColorTranslator.FromOle(Convert.ToInt32(textSymbol1.Color.RGB)).Equals(ColorTranslator.FromOle(Convert.ToInt32(textSymbol.Color.RGB))))
					{
						flag = false;
					}
