...
{
    public  class ToolSceneZoomOut : YutaiTool
    {
        ...
        public ToolSceneZoomOut(IAppContext context, IScenePlugin plugin)
        {
            OnCreate(context);
            _plugin = plugin;
        }
        public override void OnCreate(object object_0)
        {
            ...
            this.m_caption = "缩小";
            this.m_toolTip = "缩小";
            this.m_name = "Scene_ZoomOut";
            this.m_message = "缩小";
           
            this.m_bitmap = Properties.Resources.zoomout;
           
            this.cursor_0 = new System.Windows.Forms.Cursor(base.GetType().Assembly.GetManifestResourceStream("Yutai.Plugins.Scene.Resources.Cursor.ZOOMOUT.CUR"));
            this.brush_0 = new System.Drawing.SolidBrush(System.Drawing.Color.Transparent);
            this.pen_0 = new System.Drawing.Pen(System.Drawing.Color.Black, 2f);
        }
            ...
            if (this.bool_0)
            {
                try
                {
                    ...
                    if (envelope.Width == 0.0 || envelope.Height == 0.0)
                    {
                        ...
                        camera.Zoom(1.3333333333333);
                    }
                    else
                    {
                        System.Drawing.Rectangle rectangle = default(System.Drawing.Rectangle);
                        if (ToolSceneZoomOut.GetWindowRect(this._plugin.ActiveViewer.hWnd, ref rectangle) == 0)
                        {
                            return;
                        }
                        if (camera.ProjectionType == esri3DProjectionType.esriPerspectiveProjection)
                        {
                            double num = (double)Math.Abs(rectangle.Right - rectangle.Left) * ((double)Math.Abs(rectangle.Right - rectangle.Left) / envelope.Width);
                            double num2 = (double)Math.Abs(rectangle.Top - rectangle.Bottom) * ((double)Math.Abs(rectangle.Top - rectangle.Bottom) / envelope.Height);
                            IPoint point = new Point();
                            point.PutCoords(envelope.XMin + envelope.Width / 2.0, envelope.YMin + envelope.Height / 2.0);
                            envelope.XMin = point.X - num / 2.0;
                            envelope.YMin = point.Y - num2 / 2.0;
                            envelope.Width = num;
                        ...
                        else
                        {
                            ...
                            double num3 = envelope.Width;
                            double num4 = envelope.Height;
                            if (num3 > 0.0 && num4 > 0.0)
                            {
                                num3 = (double)Math.Abs(rectangle.Right - rectangle.Left) / num3;
                                num4 = (double)Math.Abs(rectangle.Top - rectangle.Bottom) / num4;
                                if (num3 < num4)
                                {
                                    camera.Zoom(num3);
                                }
