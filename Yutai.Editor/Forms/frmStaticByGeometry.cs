...
namespace Yutai.Plugins.Editor.Forms
{
    ...
    public class frmStaticByGeometry : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                str = string.Concat(this.listView1.Items[i].SubItems[0].Text, "|",
            ...
            for (i = 0; i < arrayLists.Count; i++)
            {
                ...
                this.listView2.Columns.Add(string.Concat(strArrays2[1], "_", strArrays2[0]), 120,
            ...
            {
                IList lists = null;
                lists = (this.cboClassField.SelectedIndex != 0
                    ? this.FeatureClassOverlapStatistics(featureClass, this.cboClassField.Text, this.m_pPolygon,
                        strArrays, strArrays1)
                    : this.FeatureClassOverlapStatistics(featureClass, this.m_pPolygon, strArrays, strArrays1));
        ...
        private void cboLayers_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            if (this.cboLayers.SelectedIndex != -1)
            {
                ...
                for (int i = 0; i < fields.FieldCount; i++)
                {
                    ...
                    if (type == esriFieldType.esriFieldTypeInteger || type == esriFieldType.esriFieldTypeSmallInteger ||
                        type == esriFieldType.esriFieldTypeDouble || type == esriFieldType.esriFieldTypeSingle)
                    {
        ...
        /// </param>
        public IList FeatureClassOverlapStatistics(ITable pTable, string classField, IPolygon pPolygon,
            string[] FieldNames, string[] StatisticsTypes)
        {
            IList lists;
            IList arrayLists = new ArrayList();
            try
            {
                ...
                else
                {
                    ...
                    {
                        str = (type != esriFieldType.esriFieldTypeString
                            ? string.Concat(classField, " = ", arrayLists1[i].ToString())
                            : string.Concat(classField, " = '", arrayLists1[i].ToString(), "'"));
