...
namespace Yutai.Pipeline.Editor.SqlCe
{
    ...
    public class SqlCeBase : IDisposable
    {
        ...
        {   // List of available versions starting with the oldest
            ...
            new Version("3.5", "3.5.1.0", 0x00357b9d),
        ...
        private bool OpenConnection(Version version, string databaseFile, string password)
        {
            ...
            catch (Exception e)
            {
                try
                {
                    ...
                    string progFilePath = IntPtr.Size == 4 ? string.Format("{0} (x86)", Environment.SpecialFolder.ProgramFiles) : Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
        ...
        private string GetConnectionString(string databaseFile, string password)
        {
            ...
            try
            {
                size = new FileInfo(databaseFile).Length / 1048576;   // 1Mb = 1024 Kb, 1Kb = 1024 bytes
            }
            catch
            {
            }

            size = size * 2;  // To allow the database to grow to twice its current size
            if (size > 4091) size = 4091;  // Maximum 4 Gb (with value 4096 the program is not able to open databases above 2045 Mb)
            if (size > 256) connectionStr += "; Max Database Size=" + size;
