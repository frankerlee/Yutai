...
namespace Yutai.Pipeline.Analysis.Helpers
{
    public class BaseFun
    {
        ...
        public void NoteOrientationCenterText(IActiveView pView, IPolyline pPolyLine, string sNoteText)
        {
            ...
            double num2;
            double num3;
            if ((((IPointCollection) pPolyLine).get_Point(1).X < ((IPointCollection) pPolyLine).get_Point(0).X &&
                 ((IPointCollection) pPolyLine).get_Point(1).Y > ((IPointCollection) pPolyLine).get_Point(0).Y) ||
                (((IPointCollection) pPolyLine).get_Point(1).X > ((IPointCollection) pPolyLine).get_Point(0).X &&
                 ((IPointCollection) pPolyLine).get_Point(1).Y < ((IPointCollection) pPolyLine).get_Point(0).Y))
            {
