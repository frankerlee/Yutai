...
{
	public class MyGxFilterPolygonFeatureClasses : IGxObjectFilter
	{
		...
		public MyGxFilterPolygonFeatureClasses()
		{
		}
				...
				if (igxObject_0 is IGxDataset)
				{
					...
					else if (type == esriDatasetType.esriDTFeatureClass)
					{
						...
						if (datasetName.FeatureType == esriFeatureType.esriFTSimple)
						{
							...
							{
								if (((igxObject_0 as IGxDataset).Dataset as IFeatureClass).ShapeType == esriGeometryType.esriGeometryPolygon)
								{
									flag = true;
									return flag;
								}
							}
							else if (datasetName.ShapeType == esriGeometryType.esriGeometryPolygon)
							{
								flag = true;
								return flag;
							}
