...
namespace Yutai.Catalog
{
	public class GxGPServer : IGxObject, IGxObjectContainer, IGxObjectEdit, IGxObjectProperties, IGxObjectUI, IGxContextMenuWap, IGxAGSObject
	{
		...
		public string Category
		{
			...
			{
				string str;
				str = (this.iagsserverObjectName_0 == null ? "GPServer" : this.iagsserverObjectName_0.Type);
				return str;
			}
		}
		...
		public Bitmap LargeImage
		{
			get
			{
				...
				{
					smallImage = ImageLib.GetSmallImage(83);
				}
				else if (this.Status != "Stopped")
				{
					smallImage = (this.Status != "Paused" ? ImageLib.GetSmallImage(83) : ImageLib.GetSmallImage(85));
				}
				else
				{
					smallImage = ImageLib.GetSmallImage(84);
				}
				return smallImage;
			}
		}
		...
		private void method_0()
		{
			if (this.int_0 == 0)
			{
				...
				{
					IGPToolInfo element = toolInfos.Element[i];
					IGxObject gxGPGPTool = new GxGPGPTool();
					(gxGPGPTool as IGxGPGPTool).AGSServerObjectName = this.iagsserverObjectName_0;
					(gxGPGPTool as IGxGPGPTool).AGSServerConnection = this.iagsserverConnection_0;
					(gxGPGPTool as IGxGPGPTool).GPToolInfo = element;
					gxGPGPTool.Attach(this, this.igxCatalog_0);
				}
			}
		}
