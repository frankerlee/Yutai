...
namespace Yutai.Catalog
{
	public class MyGxFilterPersonalGeodatabases : IGxObjectFilter
	{
		...
		public MyGxFilterPersonalGeodatabases()
		{
		}
		...
		public bool CanDisplayObject(IGxObject igxObject_0)
		{
			...
			if ((igxObject_0 is IGxDiskConnection ? false : !(igxObject_0 is IGxFolder)))
			{
				flag = (!(igxObject_0 is IGxDatabase) || (igxObject_0 as IGxDatabase).IsRemoteDatabase || !(Path.GetExtension(igxObject_0.Name).ToLower() == ".mdb") ? false : true);
			}
			else
			{
				flag = true;
