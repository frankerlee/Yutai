    ...
    internal class FieldTypeDoubleCtrl : UserControl, IControlBaseInterface
    {
        ...
        private void FieldTypeDoubleCtrl_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (List<JLK.Utility.CodeDomainEx.CodeDomainEx>.Enumerator enumerator = CodeDomainManage.CodeDomainExs.GetEnumerator())
                {
                    while (enumerator.MoveNext())
                    {
                        JLK.Utility.CodeDomainEx.CodeDomainEx current = enumerator.Current;
        ...
        private void txPrecision_EditValueChanged(object sender, EventArgs e)
        {
            if (this.bool_0)
            {
                if (Common.IsNmuber(this.txPrecision.Text))
                {
                    this.txtScale.ForeColor = SystemColors.WindowText;
                    this.ifieldEdit_0.Precision = Convert.ToInt32(this.txPrecision.Text);
                }
                else
                {
        ...
        private void txtScale_EditValueChanged(object sender, EventArgs e)
        {
            if (this.bool_0)
            {
                if (Common.IsNmuber(this.txtScale.Text))
                {
                    this.txtScale.ForeColor = SystemColors.WindowText;
                    this.ifieldEdit_0.Scale = Convert.ToInt32(this.txtScale.Text);
                }
                else
                {
